...
export default function MessageBar({
  ...
  const [isASRActive, setIsASRActive] = useState(false); 
  ...
  return (
    <div className="p-4 border-t bg-white relative">
      ...
      <div className="flex items-center bg-blue-50 rounded-full px-4 py-2 w-full">
        ...
        {/* Active ASR or Voice Note */}
        {(isRecording || audioBlob || isASRActive) ? (
          <>
            {isRecording && (
              <div className="ml-2 text-sm text-red-600 font-medium flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-red-600 animate-ping inline-block" />
                Recording...
              </div>
            )}
            {(isRecording || isASRActive) && (
              <button
                onClick={isASRActive ? stopASR : cancelRecording}
                className="ml-2 p-2 rounded-full hover:bg-red-100 text-red-600 transition"
                title="Stop ASR or Voice"
              >
                <X className="w-5 h-5" />
              </button>
            )}
